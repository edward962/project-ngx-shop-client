				...
				switchMap(
					({ id }): Observable<Action> =>
						this.productsService.getProductById(id).pipe(
							map((product: IProduct): Action => {
								return getProductSuccess({ product });
							}),
							catchError((err: Error): Observable<Action> => of(getProductError({ err }))),
						),
				),
				...
				switchMap(
					([{ feedback }, product]): Observable<Action> =>
						this.productsService.createFeedback(feedback, product).pipe(
							tap((): ActiveToast<String> => this.toastr.info('Вы успешно добавили отзыв')),
							...
							catchError((err: Error): Observable<Action> => of(createFeedbackError({ err }))),
						),
				),
